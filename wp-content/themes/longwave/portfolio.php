    ...
    <div class="inner">	    
		...
		<?php if(have_posts()) : 
		    	while(have_posts()) : the_post();	
		    		the_content();
		    		$title = empty($pageoptions["tb_longwave_page_portfolio_title"]) ? "" : $pageoptions["tb_longwave_page_portfolio_title"];
		    		$categories = empty($pageoptions["tb_longwave_portfolio_categories"]) ? "" : implode(",", $pageoptions["tb_longwave_portfolio_categories"]);
		    		$type = empty($pageoptions["tb_longwave_portfolio_display_type"]) ? "" : $pageoptions["tb_longwave_portfolio_display_type"];
		    		echo do_shortcode('[portfolio title="'.$title.'" cat_slugs="'.$categories.'" number=99 type="'.$type.'"]');	
		    	endwhile;  //have_posts 
