<?php
	...
	elseif(is_archive()){
		wp_link_pages();
		$htitle = __('Archive', 'tb_longwave');
	}
	if(is_tag()) $htitle =  __('Tag Archive', 'tb_longwave');


	//Sidebar
		if(isset( $themeoptions["tb_longwave_blog_category_sidebar"]))
			$pageoptions["tb_longwave_sidebar"] = $themeoptions["tb_longwave_blog_category_sidebar"];
			
	//Orientation
		if(isset( $themeoptions["tb_longwave_blog_category_style"]))
			$pageoptions["tb_longwave_blog_display_type"] = $themeoptions["tb_longwave_blog_category_style"];
		else
			$pageoptions["tb_longwave_blog_display_type"]="left";
  			 ...
  			 if(empty($pageoptions["tb_longwave_page_intro"])){ ?>	
					...
					<div class="dark-wrapper page-title"> 
						...
						<div class="inner">
							...
							<?php head_pagination_arch($paged,$posts_per_page); ?>
							<div class="clear"></div>
						</div>
						<!-- End Inner --> 
					</div>
