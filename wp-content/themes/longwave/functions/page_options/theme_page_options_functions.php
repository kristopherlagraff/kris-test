			...
			//Image
					case 'image':
						...
						echo '<span class="custom_default_image" style="display:none">'.$image_def.'</span><a href="#" class="custom_media_upload">Choose Image</a><br>
								...
								<input class="custom_media_id" type="hidden" name="'.$field['id'].'" value=""><br clear="all" /><small>&nbsp;<a href="#" class="custom_clear_image_button">Remove Image</a></small><br><span class="description">'.$field['desc'].'</span></td>';
					break;
			
			case 'repeatable_partners':
						$meta_body = get_post_meta($post->ID, $field['id'], true);
						$meta_data = get_post_meta($post->ID, $field['id']."_data", true);
						$meta_limit = get_post_meta($post->ID, $field['id']."_limit", true);
						echo '<a class="repeatable-add-image button" href="#">Add</a>
						...
						if (is_array($meta_body)) {
								...
								echo '<li class="widget ui-draggable"><div class="widget-top sort hndle" style="cursor:move; height:30px;margin-bottom:2px;"><span style="line-height:30px;margin-left:10px;">|||</span><a class="repeatable-remove button" style="float:right;margin-top:3px;margin-right:10px;" href="#">Remove</a></div>
												<div style="padding:10px;">
													...
													<select name="'.$field['id'].'[]">
														<option value="facebook_page" ' . selected( $meta, 'facebook_page', false) . '>Facebook Page</option>
														<option value="twitter" ' . selected( $meta, 'twitter', false) . '>Twitter</option>
														<option value="dribbble" ' . selected( $meta, 'dribbble', false) . '>Dribbble</option>
														<option value="flickr" ' . selected( $meta, 'flickr', false) . '>Flickr</option>
														<option value="pinterest" ' . selected( $meta, 'pinterest', false) . '>Pinterest</option>
														<option value="youtube" ' . selected( $meta, 'youtube', false) . '>Youtube</option>
														<option value="vimeo" ' . selected( $meta, 'vimeo', false) . '>Vimeo</option>											
														<option value="delicious" ' . selected( $meta, 'delicious', false) . '>Delicious</option>
														<option value="tumblr" ' . selected( $meta, 'tumblr', false) . '>Tumblr</option>
														<option value="digg" ' . selected( $meta, 'digg', false) . '>Digg</option>
													</select>
													<table width="80%">
														<tr><td style="padding:0;">Account</td><td style="padding:0;">Limit</td></tr>
														<tr><td width="70%" width="70%" style="padding:0;padding-right:8px;"><input class="widefat" style="width:100%" name="'.$field['id'].'_data[]" value="'.$meta_data[$i].'"></td><td  style="padding:0;" width="10%"><input class="widefat" style="width:100%" name="'.$field['id'].'_limit[]" value="'.$meta_limit[$i].'"></td>
													</table>
												</div>
											</li>';
							...
							echo '<li class="widget ui-draggable"><div class="widget-top sort hndle" style="cursor:move; height:30px;margin-bottom:2px;"><span style="line-height:30px;margin-left:10px;">|||</span><a class="repeatable-remove button" style="float:right;margin-top:3px;margin-right:10px;" href="#">Remove</a></div>
										<div style="padding:10px;">
											...
											<table width="80%">
												<tr><td style="padding:0;">Account</td><td style="padding:0;">Limit</td></tr>
												<tr><td width="70%" style="padding:0;padding-right:8px;"><input class="widefat" style="width:100%" name="'.$field['id'].'_data[]" value="#"></td><td  style="padding:0;" width="10%"><input class="widefat" style="width:100%" name="'.$field['id'].'_limit[]" value="4"></td>
											</table>
										</div>
									</li>';
						}
...
function save_custom_meta($post_id) {
    global $custom_meta_fields,$custom_post_portfolio_type_meta_fields,$custom_post_video_type_meta_fields,$custom_page_video_meta_fields,$custom_page_blog_meta_fields,$custom_page_portfolio_meta_fields,$custom_page_meta_fields,$custom_post_meta_fields,$custom_portfolio_meta_fields,$custom_post_type_meta_fields,$custom_page_gallery_meta_fields;
    if(isset($_POST['post_type'])){
	    // which fields to use
	    if ('page' == $_POST['post_type']) {
			$custom_meta_fields = array_merge($custom_page_meta_fields,$custom_page_blog_meta_fields,$custom_page_portfolio_meta_fields);
		}
		...
		foreach ($custom_meta_fields as $field) {
			if($field['type'] == 'tax_select') continue;
				...
				//echo "<br><br> ".$field['id']." > "; print_r($new);
