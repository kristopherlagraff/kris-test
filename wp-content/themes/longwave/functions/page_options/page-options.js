...
jQuery("document").ready(function(){
	...
	jQuery("#page_template").change(function(){
			jQuery(".tp_options").each(function(){
				if(!jQuery(this).hasClass(jQuery("#page_template").val().replace(".php", ""))){
					...
					if(jQuery("#tb_longwave_activate_sidebar").attr("checked")){
						jQuery(".sidebar").show();
					}
					else{
						jQuery(".sidebar").hide();
		...
		if(jQuery("#page_template").val().replace(".php", "")!="index")
	...
	jQuery("#tb_longwave_portfolio_excerpt_active").click(function() {
	...
	jQuery("#tb_longwave_activate_sidebar").click(function() {
		...
		if($this){
			...
			if(jQuery("input[name=tb_longwave_portfolio_items_row]:checked").val()=="one"){
	...
	jQuery("#tb_longwave_activate_page_title").click(function(){	
		if(jQuery("#tb_longwave_activate_page_title").attr("checked")) {
...
//on load page template options show/hide
	...
	if(jQuery("#page_template").length){
	 	if(jQuery("#page_template").val().replace(".php", "")=="portfolio"){
	 		if(jQuery("#tb_longwave_portfolio_excerpt_active").attr("checked")){
	 			jQuery(".excerpt").show();
	 		}
	 		else{
	 			jQuery(".excerpt").hide();
	...
	jQuery("input[name=\"tb_longwave_post_type\"]").click(function(){
...
jQuery("input[name=\"tb_longwave_video_type\"]").click(function(){
		postType = jQuery(this).val();
		jQuery(".post_type").each(function(){
			$this=jQuery(this);
			if($this.hasClass(postType)) $this.show();
...
//post video options onclick
jQuery("input[name=\"tb_longwave_image_type\"]").click(function(){
		postType = jQuery(this).val();
		jQuery(".post_type").each(function(){
			$this=jQuery(this);
			if($this.hasClass(postType)) $this.show();
...
//onload post type options
	postType = jQuery("input[name=tb_longwave_post_type]:checked").val();
	jQuery(".post_type").each(function(){
		$this=jQuery(this);
		if($this.hasClass(postType)) $this.show();
		else $this.hide();
	...
	if(postType=="video"){
		postType = jQuery("input[name=tb_longwave_video_type]:checked").val();
		jQuery(".post_type").each(function(){
			$this=jQuery(this);
			if($this.hasClass(postType)) $this.show();
			else $this.hide();
...
jQuery("input[name=\"tb_longwave_portfolio_link\"]").click(function(){
		postType = jQuery(this).val();
...
//onload external link
	postType = jQuery("input[name=tb_longwave_portfolio_link]:checked").val();
	jQuery(".post_link").each(function(){
		$this=jQuery(this);
		if($this.hasClass(postType)) $this.show();
		else $this.hide();
...
if(jQuery("input[name=tb_longwave_video_width]").val()==""){
	jQuery("input[name=tb_longwave_video_width]").val("500");
}

if(jQuery("input[name=tb_longwave_video_height]").val()==""){
	jQuery("input[name=tb_longwave_video_height]").val("281");
}

jQuery("#tb_longwave_activate_slider").click(function() {
	$this=jQuery(this).attr("checked");
	if($this){
...
if(jQuery("#tb_longwave_activate_slider").attr("checked")){
