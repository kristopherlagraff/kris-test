...
function ST5_columns_head($defaults) {
 $defaults['tb_longwave_category'] = 'Category';
 return $defaults;
}

function ST5_columns_content($column_name, $post_ID) {
  if ($column_name == 'tb_longwave_category') {
...
// ADD NEW COLUMN
function ST6_columns_head($defaults) {
 $defaults['tb_longwave_gallery_category'] = 'Gallery';
 return $defaults;
}

function ST6_columns_content($column_name, $post_ID) {
  if ($column_name == 'tb_longwave_gallery_category') {
  	$terms = wp_get_post_terms( $post_ID, "category_gallery",array("fields" => "names"));
  	echo implode(",",$terms);
  }
}

add_filter('manage_gallery_posts_columns', 'ST6_columns_head');
add_action('manage_gallery_posts_custom_column', 'ST6_columns_content', 10, 2);
