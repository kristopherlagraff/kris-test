	...
	function getCategories($id){
		...
		if($categories){
			foreach($categories as $category) {
				$output .= '<div class="blogcategory"><a href="'.get_category_link($category->term_id ).'" title="' . esc_attr( sprintf( __( "View all posts in %s",'tb_longwave' ), $category->name ) ) . '">'.$category->cat_name.'</a></div>';
			}
		return $output;
		}
	}
...
//Get all Theme Options as Array
	function getThemeOptions(){
		if( !empty($name) && !get_option( $name ) ) {
			return array_merge(get_option("tb_longwave_theme_general_options"),get_option("tb_longwave_theme_header_options"),get_option("tb_longwave_theme_body_options"));
		} else {
...
//Get the excluded Categories from the Portfolio	
	function get_excluded_portfolio_categories($post_id){
		...
		if( !empty($page_options["tb_longwave_portfolio_categories"]) && !in_array("all", $page_options["tb_longwave_portfolio_categories"])){
			foreach ($tax_terms as $tax_term) {
				if(!in_array($tax_term->slug,$page_options["tb_longwave_portfolio_categories"]))
		...
		if ( ! empty( $excluded_categories ) ) {
			if ( ! is_array( $excluded_categories ) ) {
				...
				if ( strpos( $excluded_categories, ' and ' ) !== false ) {
					_deprecated_argument( __FUNCTION__, '3.3', sprintf( __( 'Use commas instead of %s to separate excluded categories.','tb_longwave' ), "'and'" ) );
